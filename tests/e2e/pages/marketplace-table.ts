...
export class MarketplaceTable {
  ...
  async checkCellVerticalScroll(cellIndex: number) {
    ...
    if (scrollHeight > clientHeight) {
      ...
      const overflowY = await cellElement.evaluate((el) =>
  ...
  async testCellVerticalScroll() {
    ...
    const longContent = Array.from({ length: 30 }, (_, i) => `Line ${i + 1}: This is a long line of text that will wrap and create multiple lines.`.repeat(2)).join('\n');
  ...
  async testMultipleCellsWithVerticalScroll() {
    ...
    const longContent =
      'This is a very long line of text that will wrap and create multiple lines. '.repeat(10);
